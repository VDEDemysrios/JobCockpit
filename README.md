...
> **Pressé ?** Télécharge l'exécutable Windows dans les
> [versions publiées](https://github.com/VDEDemysrios/JobCockpit/releases),
> décompresse, double-clique. Node n'est pas nécessaire, et un assistant
...
- **📄 Mon CV** — **le document que tu envoies**, à ouvrir d'un clic : c'est lui,
  ...
  extraction.
- **⚙️ Options** — **tes villes** en premier (voir §6), puis densité,
  animations, thème, délai de relance, objectif hebdomadaire, état des sources
  et export de toutes tes données en JSON.
...
**Clair** · **EnR** · **Nuit** · **Cockpit** (fonds froids, accent ambre,
pensé pour les sessions du soir). Touche `T` pour tourner.

### L'ouverture

Au lancement, un **balayage radar** passe sur une centaine de points — les
offres du marché. La plupart s'allument puis s'éteignent : collectées,
examinées, écartées. Trois restent et pulsent, comme la sélection du jour.
C'est le travail de l'application, montré en trois secondes et demie.

Elle ne joue qu'une fois par ouverture — un `F5` en pleine session ne la
rejoue pas — et se saute d'un clic ou d'une touche. Elle se coupe aussi si tu
désactives les animations, et se désactive séparément dans les Options.

### La visite guidée

À la première ouverture, six étapes désignent les éléments réels en changeant
de vue : la sélection du jour, le bouton de collecte, les onglets de villes,
le verdict sur une carte, le dépôt du CV, la palette. Rejouable depuis les
Options ou par `Ctrl` + `K`.
...
- **Hors zone** — l'offre est en dehors des villes que tu as déclarées.
...
Deux façons d'utiliser Job Cockpit. La seconde est celle du quotidien.

### A. Depuis le terminal, pour essayer

```bash
npm start        # le tableau de bord sur http://localhost:3000
npm run collect  # une collecte, à la demande
```

Simple, mais il faut penser à lancer les deux, et fermer le terminal arrête
tout.

### B. Comme une vraie application, pour s'en servir

Job Cockpit se construit en **un exécutable autonome** — un seul fichier, qui
n'exige même pas que Node soit installé sur la machine :

```bash
npm run exe        # construit dist/JobCockpit.exe
npm run installer  # l'installe dans un dossier à côté du projet
```

L'installateur crée un dossier `Application` contenant l'exécutable, ton
`.env`, ton profil et ta base. **Il ne recopie tes données qu'à la première
installation** : les mises à jour suivantes ne remplacent que le programme.
Une mise à jour qui effacerait ton suivi de candidatures serait pire que pas
de mise à jour du tout.

Pour démarrer sans fenêtre, double-clique **`Demarrer.vbs`** dans ce dossier —
ou place un raccourci vers ce fichier dans le dossier « Démarrage » de Windows
(touche Windows + R, puis `shell:startup`) pour qu'il se lance à l'ouverture
de session.

### La collecte tourne dans le serveur

`COLLECTE_AUTO=1`, posé par `Demarrer.vbs`, fait collecter le serveur
lui-même : **une fois au démarrage, puis toutes les 6 heures**. Il n'y a donc
qu'une seule chose à lancer, et plus aucune tâche planifiée à entretenir.

> Une collecte de moins d'une heure n'est pas refaite au redémarrage : relancer
> l'application dix fois dans la journée ne consomme pas dix fois le quota.

Le compte rendu de chaque passage s'écrit dans `journal.log`, à côté de
l'exécutable. Il est vidé à chaque démarrage : ce qu'on y cherche est toujours
à la fin.

Le bouton **« Collecter »** du bandeau reste disponible quand tu ne veux pas
attendre.

### Si l'application ne démarre pas

Une seule instance peut tourner à la fois — deux serveurs sur la même base,
ce serait deux collectes simultanées et deux fois le quota dépensé.
`Demarrer.vbs` refuse donc de lancer un second exemplaire, et le serveur
s'arrête de lui-même s'il trouve le port déjà pris, en le disant dans le
journal.

L'installateur, lui, vérifie que l'application est **réellement repartie**
après une mise à jour, et le signale si ce n'est pas le cas.

### Linux / macOS

Il n'y a pas d'exécutable : on lance le serveur avec `COLLECTE_AUTO=1`, et il
se charge du reste.

```bash
COLLECTE_AUTO=1 npm start
```

Pour qu'il démarre avec la session, un service `systemd` ou un
`@reboot` dans `crontab -e` suffit.

---

## 6. Régler le classement des offres

### Tes villes, depuis l'application

**Options → Mes villes.** Ajoute, retire ou renomme tes villes cibles ; un nom
et un code postal suffisent. Déménager ou abandonner une ville ne demande pas
de rouvrir un fichier.

Chaque ville a deux périmètres, repliés sous « Périmètre » et à ne toucher que
si un onglet finit par contenir des annonces qui n'ont rien à y faire :

| | Ce que ça commande | Exemple |
|---|---|---|
| **L'onglet affiche** | où l'offre se range | Nancy → 54 seulement |
| **La collecte cherche** | ce qu'on accepte de considérer | 54, 57, 88, 55 |

Les confondre est ce qui mettait Metz dans l'onglet Nancy : un onglet qui porte
un nom de ville et contient un département entier ne range plus rien. Les
communes de l'agglomération dont le nom ne contient pas celui de la ville —
Villeurbanne pour Lyon — se déclarent à part : rien ne permet de les deviner.

### Le reste, dans `profile/profile.json`

Sans toucher au code :

- **`flux`** — les flux RSS à surveiller (voir juste en dessous)
- **`intitules`** — les intitulés de poste recherchés
...
1. Pour chacun de tes intitulés, il interroge chaque source sur chacune de tes villes
...
### Le mot de passe est obligatoire, et le programme y veille

Une porte d'entrée par mot de passe existe : cookie signé, blocage après 8
tentatives, en-têtes de sécurité stricts. Elle s'active dès que
`COCKPIT_MOT_DE_PASSE` est renseigné.

Et **le serveur refuse de démarrer** s'il doit écouter sur une adresse
publique sans mot de passe. Ce n'est pas un avertissement qu'on peut ignorer :
sans cette règle, une variable d'environnement oubliée publierait le CV, les
candidatures et les lettres.

```bash
fly secrets set COCKPIT_MOT_DE_PASSE="…"
```

### Avant de te lancer, deux choses à peser

- **Les quotas des API sont attachés à ta clé.** Adzuna offre 1 000 appels par
  mois ; une configuration à 6 intitulés et 4 villes en consomme environ 900 à
  elle seule. Gemini se compte **par jour**. Héberger l'outil pour d'autres
  personnes épuiserait donc *ton* quota — chacun doit avoir ses propres clés.
- **Si tu veux seulement y accéder depuis ton téléphone**, un réseau privé
  (type Tailscale) suffit et coûte moins cher qu'un hébergement : la base reste
  sur ton ordinateur, rien n'est exposé à Internet. L'hébergement ne se
  justifie que pour collecter **ordinateur éteint**.
