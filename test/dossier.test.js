...
const COORD = { nom: 'Benjamin Perrin', email: 'b@exemple.fr', tel: '06 00 00 00 00', ville: 'Épinal' };
...
test('le dossier contient la lettre ET le CV', async () => {
  const zip = await JSZip.loadAsync(await construireDossier({
    ...
    cv: { nom: 'CV_Benjamin_Perrin.docx', contenu: Buffer.from('un vrai CV') },
  }));
  ...
  assert.ok(fichiers.includes('CV_Benjamin_Perrin.docx'), `CV absent : ${fichiers}`);
});
